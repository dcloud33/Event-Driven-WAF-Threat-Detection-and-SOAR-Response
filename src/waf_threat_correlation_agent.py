...
def native_to_decimal(value: Any) -> Any:
    """
    Convert Python float values into Decimal values recursively.

    DynamoDB does not support Python float values, including floats
    nested inside dictionaries or lists.
    """
...
def get_recent_events(
    ...
    The WAF analyzer stores:
    - timestamp as an ISO-8601 UTC string
    - event_epoch as Unix epoch time in seconds
    """
    ...
    for item in items:
        native_item = decimal_to_native(item)
        raw_event_epoch = native_item.get("event_epoch")

        if raw_event_epoch is None:
            print(
                f"Skipping event "
                f"{native_item.get('event_id', 'UNKNOWN')}: "
                "event_epoch is missing."
            )
            continue

        try:
            event_epoch = int(raw_event_epoch)
            ...
            print(
                f"Skipping event "
                f"{native_item.get('event_id', 'UNKNOWN')}: "
                f"invalid event_epoch {raw_event_epoch}. "
                f"Error: {error}"
            )
            continue

        if window_start <= event_time <= window_end:
            native_item["event_epoch"] = event_epoch
            native_item["event_time"] = event_time.isoformat()
            recent_events.append(native_item)
    ...
    print(
        f"Retrieved {len(recent_events)} event(s) "
        "inside the correlation window."
    )

    return recent_events, window_start, window_end
        ...
        correlations.append(
            {
                ...
                "event_count": event_count,
                "blocked_count": blocked_count,
                "allowed_count": event_count - blocked_count,
                "unique_uris": len(uris),
                "uris": sorted(uris),
                "unique_rules": len(rules),
                "rules": sorted(rules),
                "countries": sorted(countries),
                "first_seen": source_events[0].get("timestamp"),
                "last_seen": source_events[-1].get("timestamp"),
                "active_span_minutes": active_span_minutes,
                "sensitive_uri_targeted": sensitive_uri_targeted,
        ...
        target_correlations.append(
            {
                ...
                "sensitive_uri": contains_sensitive_uri(uri),
            }
        )
...
def save_finding(
    ...
    dynamodb_item = native_to_decimal(item)

    findings_table.put_item(Item=dynamodb_item)
...
def lambda_handler(
    ...
    try:
        events, window_start, window_end = get_recent_events(
            window_minutes
        )
        ...
        bedrock_report = call_bedrock(evidence_package)
        ...
        return {
            ...
            "body": json.dumps(
                {
                    "message": (
                        "Threat correlation failed because "
